...
do
    ...
    if [[ $arg1 == *keys* || $arg1 == *all* ]] ; then
      ...
      ssh $resource_domain_server "nsc_sh 'cd /root/.ssh; [ -f authorized_keys.tsctl2.backup ] || cp authorized_keys authorized_keys.tsctl2.backup' $target"  > /dev/null 2>&1
      ssh $resource_domain_server "nsc_sh 'cd /root/.ssh; cat authorized_keys.tsctl2.backup aks > authorized_keys' $target"  > /dev/null 2>&1
    fi
