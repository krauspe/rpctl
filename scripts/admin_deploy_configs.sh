...
#
#TODO: divide between SLES and Centos systems: HIER!!!
# SLES: deploy as is
# CentOS: - GET ONLY hiera data: get hiera data from each resource nsc from resource domain(nss):
#              rsync -ahH nss.<resource_dn>:/opt/export/puppet-master/hieradata/<resource_dn> <admin_host>
#                                   get hiera data from each remote nsc from remote domain(nss):
#                                                      deside: from DNS txt record as before OR ??
#         - scripts are (will be) integrated in rpctl and dfs-remote_adm package
